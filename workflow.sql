...
WHERE EXISTS 
	(SELECT *
	FROM TS_OTHER_TEST_statements_md5 st, TS_OTHER_TEST_suggestions_md5 sug
	WHERE sug.subject_md5 = st.subject_md5
	AND sug.predicate_md5 = st.predicate_md5
	AND sug.predicate_md5 IN (SELECT predicate_md5 FROM TS_OTHER_TEST_property_stats_md5 WHERE considered_functional = 1));
