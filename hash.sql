...
DROP TABLE IF EXISTS `categories_to_md5`;
CREATE TABLE `categories_to_md5` (
  `resource` char(32) NOT NULL,
  `category` char(32) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

DROP TABLE IF EXISTS `statements_to_md5`;
CREATE TABLE `statements_to_md5` (
...
INSERT INTO categories_to_md5 SELECT md5(resource),md5(category) FROM  categories_original;

ALTER TABLE `categories_to_md5` 
ADD INDEX `idx_categories_resource` (`resource` ASC),
ADD INDEX `idx_categories_category` (`category` ASC);



INSERT INTO statements_to_md5 SELECT md5(subject), md5(predicate), md5(object) FROM statements_original;

ALTER TABLE `statements_to_md5` 
...
CREATE TABLE `subject_to_md5` (
  ...
  key 			`idx_subject_to_md5` (`subject`(255))
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `object_to_md5` (
  ...
  key 			`idx_object_to_md5` (`object`(255))
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

INSERT IGNORE INTO object_to_md5 SELECT subject,md5(subject) FROM statements_original;
...
CREATE TABLE `predicate_to_md5` (
  ...
  PRIMARY KEY 		`idx_predicate_to_md5_category_md5` (`predicate_md5`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
